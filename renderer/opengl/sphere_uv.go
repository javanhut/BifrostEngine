...
func NewSphereMeshWithUV() *Mesh {
	...
	// Generate sphere vertices with UV coordinates
	for ring := 0; ring <= rings; ring++ {
		...
		for segment := 0; segment <= segments; segment++ {
			...
			// V goes from top to bottom (latitude)
			u := float32(segment) / float32(segments)
